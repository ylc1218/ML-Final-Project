...
log_file_name = timeStr+"_svm.log"
...
for param in params:
	...
	# svm
	svc = svm.SVC(C=param[0], gamma=param[1], cache_size=4096, max_iter=1000000)
	cvScores = CV.cross_val_score(svc, trainingX, trainingY, scoring=scoring, cv=5,  n_jobs=5, fit_params=fit_params)
...
svc = svm.SVC(C=bestParam[0], gamma=bestParam[1], cache_size=4096, max_iter=1000000)
svc.fit(trainingX, trainingY, sample_weight=sampleWeight)
predictLabel = svc.predict(testingX) if track == 'track2' else svc.predict_proba(testingX)

print(predictLabel)

# output result to file
with open(output_fName+'_svm_'+track+'.csv', 'w') as csvfile:
