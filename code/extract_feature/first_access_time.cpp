...
int main(){
    freopen("train_mintime_maxtime.csv", "r", stdin);
    FILE* out = fopen("train_first_access.csv", "w");
    ...
    scanf("%*s,%*s,%*s,%*s");
    scanf("%d,%d-%d-%dT%d:%d:%d,%*s,%*s", &eid, &y, &mth, &d, &h, &m, &s);
    first_time = {s, m, h, d, mth-1, y-1900};

    while(~scanf("%d,%d-%d-%dT%d:%d:%d,%*s,%*s", &eid, &y, &mth, &d, &h, &m, &s)){
