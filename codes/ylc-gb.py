...
for param in params:
	...
	# gradient boost
	gbc = ensemble.GradientBoostingClassifier(n_estimators=param[0], max_depth=param[1], max_features=param[2], random_state=1126)
...
#fit
gbc = ensemble.GradientBoostingClassifier(n_estimators=bestParam[0], max_depth=bestParam[1], max_features=bestParam[2],  random_state=1126)
gbc.fit(trainingX, trainingY)
predictLabel = gbc.predict(testingX) if track == 'track2' else gbc.predict_proba(testingX)

print(predictLabel)
print >>log_f, gbc.feature_importances_ 

# output result to file
with open(output_fName+'_gb_'+track+'.csv', 'w') as csvfile:
