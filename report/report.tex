...
prediction and how we evaluate them. All the following expriments
...
penalties, hence we assign sample weights in track2 by the following
approach.
\end{itemize}
...
By adjusting the penalty paremeter $C$ and $\gamma$ in RBF kernel
...
Methods we used can be divided into three main categories: 
\begin{enumerate}
\item Clustering: Group similar enrollments into the same group, train each
group individually and combine the results. 
\item Blending: Use techniques such as Uniform blending and Stacking to
blend outcomes from different models. 
\item Predict track 2 by results of track 1: Outputs of track 1 are real
numbers between {[}0,1{]}. A naïve way to convert probability into
track 2's 0/1 prediction is using 0.5 as threshold. We put effort
in obtaining a better threshold. 
\end{enumerate}
...
result (with hightest public score) together with track 2's best result
by track 1's prediction. We wanted to set the threshold at point \textit{p}
where enrollments were mainly predicted as 0 for track 2 while track
1's predicted probabilities are less than \textit{p}. 
...
In this section we evalutate the efficiency, scalability and performance
of each model.
...
RF'' means RF paralleled with five cores while ``RF'' is not paralled. 
...
\begin{figure}[h]
\includegraphics[width=1\columnwidth]{/Users/EJHSU/Desktop/figure1}\caption{Running time of each model with different number of training data}
...
\begin{figure}[h]
\includegraphics[width=1\columnwidth]{/Users/EJHSU/Desktop/figure3}\caption{Ratio of time each model spent with different number of training data}


\centering{}\label{Figure: ratio of time each model spent on each training data}
\end{figure}
\begin{figure}[h]
\includegraphics[width=1\columnwidth]{/Users/EJHSU/Desktop/figure2}\caption{ratio of time each model spent with different number of training data
(without SVM)}
...
the efficiency, scalability, performace between different models.
...
\bibitem{key-6}\end{thebibliography}
